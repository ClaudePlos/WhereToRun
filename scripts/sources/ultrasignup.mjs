 ...
 */
export function isInteresting(distances, categories) {
  if (/\bultra\b/i.test(String(categories ?? '').replace(/non[\s-]*ultra/gi, ''))) return true;
  return isNotableDistance(distances);
}
...
export function parseEvents(json, { today = new Date() } = {}) {
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      start: { lat: point.lat, lon: point.lon, name: null, precision: 'exact' },
